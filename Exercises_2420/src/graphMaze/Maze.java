...
public class Maze {
	...
	private void generate() {
		...
		/*
		 * // delete some random walls for (int i = 0; i < rows; i++) { int col = 1 +
		 * StdRandom.uniformInt(cols - 1); int row = 1 + StdRandom.uniformInt(rows - 1);
		 * north[col][row] = false; south[col][row + 1] = false; }
		 * 
		 * // add some random walls for (int i = 0; i < cols; i++) { int col = cols / 2
		 * + StdRandom.uniformInt(cols / 2); int row = rows / 2 +
		 * StdRandom.uniformInt(rows / 2); east[col][row] = west[col + 1][row] = true; }
		 */
	}
	...
	private void solve(int col, int row) {
		...
		StdDraw.filledSquare(col + 0.5, row + 0.5, 0.25);
		StdDraw.show();
		StdDraw.pause(15);
		...
		StdDraw.filledSquare(col + 0.5, row + 0.5, 0.25);
		StdDraw.setPenColor(StdDraw.BLUE);
		StdDraw.square(col + 0.5, row + 0.5, 0.25);
		StdDraw.show();
		StdDraw.pause(15);
	}
	...
	public void draw() {
		...
		StdDraw.filledSquare(targetCol + 0.5, targetRow + 0.5, 0.375);
		StdDraw.filledSquare(1.5, 1.5, 0.375);
	...
	public static void main(String[] args) {
		int cols = 30; // Integer.parseInt(args[0]);
		int rows = 20; // Integer.parseInt(args[1]);
